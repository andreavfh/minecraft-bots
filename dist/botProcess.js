...
const triggeredResponses = new Set();
...
async function runActions(actions) {
    ...
    for (const action of actions) {
        try {
            ...
            if (action.delay)
                await (0, utils_1.wait)(Number(action.delay));
            if (action.register)
...
function runBot() {
    ...
    bot.on('message', async (message) => {
        ...
        const fullText = (0, utils_1.getMessageWithAnsi)(message);
        ...
        for (const r of config.responses || []) {
            ...
            if (text.includes(condition) && (!once || !triggeredResponses.has(condition))) {
                if (once)
                    triggeredResponses.add(condition);
                runActions([r]);
            }
        }
    });
...
async function waitUntilStablePosition(bot) {
